...
public final class Utils
{
	...
	public static Collection cloneCollection (Collection aCollection)
	{
		...
		try
		{
			...
			for (Iterator theIterator = aCollection.iterator (); theIterator.hasNext ();)
			{
				...
				if (theObject instanceof PublicCloneable)
				{
					PublicCloneable theCloneable = (PublicCloneable) theObject;
	...
	public static Map cloneMap (Map aMap)
	{
		...
		try
		{
			...
			for (Iterator theIterator = aMap.entrySet ().iterator (); theIterator.hasNext ();)
			{
				...
				if (theKey instanceof PublicCloneable)
				{
					PublicCloneable theCloneable = (PublicCloneable) theKey;
					theKeyClone = theCloneable.clone();
				}
				else theKeyClone = theKey;

				if (theValue instanceof PublicCloneable)
				{
					PublicCloneable theCloneable = (PublicCloneable) theValue;
	...
	/**
	 * Returns the smallest power of 2 that is greater than the specified value. 
	 */
	public static int getFirstGreaterPow2 (int aInt)
	{
		if (aInt == 0) return 0;
		else
		{	
			int theResult = 1;
			while (aInt > theResult) theResult <<= 1;
			return theResult;
		}
	}
}